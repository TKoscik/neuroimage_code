...
#Data dependencies:
 #EPI data
  #Motion correction directory (prep files)
motionCorrDir=${researcher}/${project}/derivatives/func/prep/sub-${subject}/ses-${session}/motionCorr
  ...
  #Final moco file directories
mocoDir=${researcher}/${project}/derivatives/func/moco/moco_${atlasName}_native
  if [[ ! -d ${mocoDir} ]]; then
    mkdir -p $mocoDir
  fi
mocoAvgDir=${researcher}/${project}/derivatives/func/moco/moco_${atlasName}_native/avg
  if [[ ! -d ${mocoAvgDir} ]]; then
    mkdir -p $mocoAvgDir
  fi
  #Final segmentation directory (func space)
segDir=${researcher}/${project}/derivatives/func/segmentation
  if [[ ! -d $segDir ]]; then
    mkdir -p $segDir
  fi
...
for epi in $(echo $epiList | sed "s/,/ /g");
  ...
  ${mocoDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}+motCor.nii.gz
  ...
  $FSLDIR/bin/fslmaths ${mocoDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}+motCor.nii.gz \
  -Tmean ${mocoAvgDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}+motCorAvg.nii.gz

  #Create a mask of the motion corrected EPI, now in standard/atlas space
  $FSLDIR/bin/bet ${mocoAvgDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}+motCorAvg.nii.gz \
  ${maskDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}+motCor -m -n
  $FSLDIR/bin/fslmaths ${maskDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}+motCor_mask.nii.gz \
  -bin ${maskDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}+motCor_mask.nii.gz -odt char
  ...
  for Tissue in 'GM' 'WM' 'CSF';
    ...
    -r ${mocoAvgDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}+motCorAvg.nii.gz  \
    -o ${segDir}/sub-${subject}_ses-${session}_site-${site}_${epiBaseStub}_seg-${Tissue}.nii.gz \
    -n NearestNeighbor \
    -t ${anatToAtlasXfm}
  done
...
chgrp -R ${group} ${segDir} > /dev/null 2>&1
chmod -R g+rw ${segDir} > /dev/null 2>&1
chgrp -R ${group} ${mocoDir} > /dev/null 2>&1
chmod -R g+rw ${mocoDir} > /dev/null 2>&1
chgrp -R ${group} ${mocoAvgDir} > /dev/null 2>&1
chmod -R g+rw ${mocoAvgDir} > /dev/null 2>&1
