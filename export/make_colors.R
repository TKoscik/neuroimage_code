...
color.palette <- unlist(strsplit(color.palette, split=":"))
n.palette <- length(color.palette)
color.order <- unlist(strsplit(color.order, split=","))
if (length(color.order) == 1) {
  color.order <- rep(color.order, n.palette)
}

color.ls <- character(0)
for (i in 1:n.palette) {
  cpal <- character(0)
  if (color.palette[i] == "timbow") {
  ...
  if (color.palette[i] %in% c("viridis", "magma", "inferno", "plasma", "cividis")) {
    color.ls <- viridis(color.n, option=color.palette[i])
  }
  if (grepl("cubehelix", color.palette[i])) {
    temp <- unlist(strsplit(color.palette[i], split=","))
    start <- 0.5
    r <- -1.5
    hue <- 2
    gamma <- 1
  ...
  if (color.palette[i] == "hot") {
    cpal <- colorRampPalette(c("#7F0000", "#FF0000", "#FF7F00", "#FFFF00", "#FFFF7F"))
  }
  if (color.palette[i] == "cold") {
    cpal <- colorRampPalette(rev(c("#00007F", "#0000FF", "#007FFF", "#00FFFF", "#7FFFFF")))
  }
  if (color.palette[i] %in% c("grayscale", "grayscale", "gray", "grey")) {
    cpal <- colorRampPalette(c("#000000", "#FFFFFF"))
  }
  if (color.palette[i] == "rainbow") {
    cpal <- colorRampPalette(c("#FF0000", "#FFFF00", "#00FF00", "#00FFFF", "#0000FF", "#FF00FF"))
  }
  if (length(cpal) == 0) {
    cpal <- colorRampPalette(unlist(strsplit(color.palette[i], split=",")))
  }
  tls <- cpal(color.n)
  
  if (color.order[i] %in% c("r", "rand", "random")) {
    tls <- sample(tls, color.n, replace = F)
  }
  if (color.order[i] %in% c("rev", "reverse", "inverse")) {
    tls <- rev(tls)
  }
  color.ls <- c(color.ls, cpal(color.n))
  ...
  ggsave(filename = paste0("cbar", i, ".png"),
         path = dir.save,
         plot = color.bar,
         device = "png",
...
fid <- file(paste0(dir.save, "/temp.lut"), open="a", encoding = "UTF-8")
writeLines(c(
