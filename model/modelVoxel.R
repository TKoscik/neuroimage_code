...
model.fxn <- function(X, ...) {
  ...
  if (OUT_DIFFLSMEANS) {
    dflsmns <- difflsmeans(mdl)
    ### FDR correction
    if (!is.na(FDR_N)) {
      dflsmns$pFDR <- p.adjust(dflsmns[ ,pmatch("P", colnames(dflsmns))], method="BY", n=FDR_N)
    }
    table.to.nii(in.table = dflsmeans, coords=coords, save.dir=DIR_SAVE,
                 do.log=TRUE, model.string=FORM,
                 img.dims=img.dims, pixdim=pixdim, orient=orient)
  }
