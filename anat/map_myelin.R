...
sub.t1 <- as.numeric(read.nii.volume(args[3],1))
sub.t2 <- as.numeric(read.nii.volume(args[4],1))
roi <- as.numeric(read.nii.volume(args[5],1))
roi1 <- which(roi==1)
roi2 <- which(roi==2)
...
norm.t1[norm.t1 < 0] <- 0
t1.Xr <- get_mode(norm.t1[roi1])
t1.Yr <- get_mode(norm.t1[roi2])

qval <- quantile(norm.t2, 0.98)
norm.t2[norm.t2 > qval] <- qval
norm.t2[norm.t2 < 0] <- 0
t2.Xr <- get_mode(norm.t2[roi1])
t2.Yr <- get_mode(norm.t2[roi2])

qval <- quantile(sub.t1, 0.98)
sub.t1[sub.t1 > qval] <- qval
sub.t1[sub.t1 < 0] <- 0
t1.Xs <- get_mode(sub.t1[roi1])
t1.Ys <- get_mode(sub.t1[roi2])

qval <- quantile(sub.t2, 0.98)
sub.t2[sub.t2 > qval] <- qval
sub.t2[sub.t2 < 0] <- 0
t2.Xs <- get_mode(sub.t2[roi1])
t2.Ys <- get_mode(sub.t2[roi2])
