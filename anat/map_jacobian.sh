...
xfm:,from:,to:,dir-save:,dir-scratch:,dir-nimgcore:,dir-pincsource:,\
...
if [[ "${HELP}" == "true" ]]; then
  ...
  echo '                           are applied in (ANTs convention)'
...
# allow multiple transforms, build stack
# automatically extract from and to, from must come from 1st xfm, to must come from last xfm
...
CreateJacobianDeterminantImage 3 \
  ${dir_xfm}/${prefix}_from-${moving}_to-${fixed}_xfm-stack.nii.gz \
  ${dir_jac}/${prefix}_from-${moving}_to-${fixed}_jac.nii.gz
